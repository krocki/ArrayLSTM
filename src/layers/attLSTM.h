 ...
 *
 * Implementation of soft attention layer
 * Described in report in Section 4.1: Lane selection, Soft attention
 *
 */
...
template <typename T>
class attLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		attLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::cout << "attlstm levels :" << _L << std::endl;
			/*init*/
			matrix_init ( p ( W ) );
			matrix_init ( p ( U ) );
			//rands = T ( _B, 4 * _N * _L );
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		/* optional */
		/* copy constr */
		//attLSTM( const attLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//attLSTM& operator=( const attLSTM& src) { Timelayer::operator=(src); return *this; }
