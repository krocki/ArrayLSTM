...
*
*   Stochastic Array Memory
*	Described in TR in Section 5.2 (Stochastic Array Memory, variant with 1/2 cells active)
*
*/
...
template <typename T>
class doLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		doLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::cout << "dolstm levels :" << _L << std::endl;
			/*init*/
			matrix_init ( p ( W ) );
			matrix_init ( p ( U ) );
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			//fused
			cu_elementwise_dolstm_forward (	& ( s ( t, g ).cu_data[0] ),
											& ( s ( t, g2 ).cu_data[0] ),
											& ( p ( b ).cu_data[0] ),
											& ( s ( t, h ).cu_data[0] ),
											& ( s ( t, max_o ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_dolstm_backward (	& ( g ( t, g ).cu_data[0] ),
												& ( g ( t, y ).cu_data[0] ),
												& ( s ( t, c ).cu_data[0] ),
												& ( s ( t, ct ).cu_data[0] ),
												& ( g ( t, c ).cu_data[0] ),
		...
		/* optional */
		/* copy constr */
		//doLSTM( const doLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//doLSTM& operator=( const doLSTM& src) { Timelayer::operator=(src); return *this; }
