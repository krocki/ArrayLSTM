 ...
 *
 * Implementation of Stack-LSTM
 * Technical Report, Figure 7.4a
 */
...
template <typename T>
class hLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		hLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::cout << "hlstm levels :" << _L << std::endl;
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			sync_stream ( 2 );
			//fused
			cu_elementwise_hlstm_forward (	& ( s ( t, g ).cu_data[0] ),
											& ( s ( t, g2 ).cu_data[0] ),
											& ( p ( b ).cu_data[0] ),
											& ( s ( t, h ).cu_data[0] ),
											& ( s ( t, c ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_hlstm_backward (	& ( g ( t, g ).cu_data[0] ),
											& ( g ( t, y ).cu_data[0] ),
											& ( s ( t, c ).cu_data[0] ),
											& ( s ( t, ct ).cu_data[0] ),
											& ( g ( t, c ).cu_data[0] ),
			...
			/* sparsify layers */
			if ( t == 1 )
				cu_elementwise_sparselstm_sparsity ( N, L, g ( t, c ).rows(), S, & ( p ( b ).cu_data[0] ), 0.05f );
		...
		/* optional */
		/* copy constr */
		//hLSTM( const hLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//hLSTM& operator=( const hLSTM& src) { Timelayer::operator=(src); return *this; }
