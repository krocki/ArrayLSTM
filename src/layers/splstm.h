...
*
*   Implementation of spLSTM layer
*
*/
...
template <typename T>
class spLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		spLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::cout << "splstm levels :" << _L << std::endl;
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			//fused
			cu_elementwise_splstm_forward (	& ( s ( t, g ).cu_data[0] ),
											& ( s ( t, g2 ).cu_data[0] ),
											& ( p ( b ).cu_data[0] ),
											& ( s ( t, h ).cu_data[0] ),
											& ( s ( t, max_o ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_splstm_backward (	& ( g ( t, g ).cu_data[0] ),
												& ( g ( t, y ).cu_data[0] ),
												& ( s ( t, c ).cu_data[0] ),
												& ( s ( t, ct ).cu_data[0] ),
												& ( g ( t, c ).cu_data[0] ),
		...
		/* optional */
		/* copy constr */
		//spLSTM( const spLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//spLSTM& operator=( const spLSTM& src) { Timelayer::operator=(src); return *this; }
