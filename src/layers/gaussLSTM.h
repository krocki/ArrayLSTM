...
*
*   Implementation of gaussLSTM layer
*
*/

#ifndef _gaussLSTM_H_
#define _gaussLSTM_H_
...
template <typename T>
class gaussLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		gaussLSTM ( size_t _M, size_t _N, size_t _B,
					size_t _S ) :
...
#define o_gates 1 * N * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			cu_randn ( rands.cu_data, rands.size(), 0.0f, 0.01f );
			
			//fused
			cu_elementwise_gauss_lstm_forward (	& ( s ( t, g ).cu_data[0] ),
												& ( s ( t, g2 ).cu_data[0] ),
												& ( p ( b ).cu_data[0] ),
												& ( s ( t, h ).cu_data[0] ),
												& ( s ( t, c ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_gauss_lstm_backward (	& ( g ( t, g ).cu_data[0] ),
													& ( g ( t, y ).cu_data[0] ),
													& ( s ( t, c ).cu_data[0] ),
													& ( s ( t, ct ).cu_data[0] ),
													& ( g ( t, c ).cu_data[0] ),
		...
		/* optional */
		/* copy constr */
		//gaussLSTM( const gaussLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//gaussLSTM& operator=( const gaussLSTM& src) { Timelayer::operator=(src); return *this; }
