...
*
*   Implementation of cmLSTM layer
*
*/
...
template <typename T>
class cmLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		cmLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::cout << "cmlstm levels :" << _L << std::endl;
			/*init*/
			matrix_init ( p ( W ) );
			matrix_init ( p ( U ) );
			rands = T ( _B, 4 * _N * _L );
			...
			p ( b ).block_forall ( 2 * N * L, N * L, [ = ] () { return -5; } );
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			//fused
			cu_elementwise_cmlstm_forward (	& ( s ( t, g ).cu_data[0] ),
											& ( s ( t, g2 ).cu_data[0] ),
											& ( s ( t, G ).cu_data[0] ),
											& ( p ( b ).cu_data[0] ),
											& ( s ( t, h ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_cmlstm_backward (	& ( g ( t, g ).cu_data[0] ),
												& ( g ( t, y ).cu_data[0] ),
												& ( s ( t, c ).cu_data[0] ),
												& ( g ( t, c ).cu_data[0] ),
												& ( s ( t, g ).cu_data[0] ),
		...
		/* optional */
		/* copy constr */
		//cmLSTM( const cmLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//cmLSTM& operator=( const cmLSTM& src) { Timelayer::operator=(src); return *this; }
