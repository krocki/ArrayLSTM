 ...
 *
 * CUDA Implementation of LSTM (baseline) with DO v2 layer
 *
 */
...
#define o_gates 1 * N * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			if ( apply_dropout ) {
				...
				cu_cmp_matrix ( dropout_mask.cu_data, rands.cu_data, s ( t - 1, h ).cu_data, rands.size() );
				cu_elementwise_mult ( s ( t - 1, h ).cu_data, dropout_mask.cu_data, s ( t - 1, h ).cu_data, dropout_mask.size() );
