 ...
 *
 * Implementation of acLSTM layer
 *
 */
...
template <typename T>
class acLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		acLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::make_tuple ( "g", _B, ( 4 + _L ) * _N * 1 ),
			std::make_tuple ( "g2", _B, ( 4 + _L ) * _N * 1 )
			
		}, {
		
			/* define params */
			std::make_tuple ( "W", _M, ( 4 + _L ) * _N * 1 ),
			std::make_tuple ( "U", _N, ( 4 + _L ) * _N * 1 ),
			std::make_tuple ( "b", 1, ( 4 + _L ) * _N * 1 ),
			
			std::make_tuple ( "B_ones", _B, 1 )
			
		} ) {
		
			std::cout << "aclstm levels :" << _L << std::endl;
			/*init*/
			matrix_init ( p ( W ) );
			matrix_init ( p ( U ) );
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			//fused
			cu_elementwise_aclstm_forward (	& ( s ( t, g ).cu_data[0] ),
											& ( s ( t, g2 ).cu_data[0] ),
											& ( p ( b ).cu_data[0] ),
											& ( s ( t, h ).cu_data[0] ),
											& ( s ( t, max_o ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_aclstm_backward (	& ( g ( t, g ).cu_data[0] ),
												& ( g ( t, y ).cu_data[0] ),
												& ( s ( t, c ).cu_data[0] ),
												& ( g ( t, c ).cu_data[0] ),
												& ( s ( t, g ).cu_data[0] ),
		...
		/* optional */
		/* copy constr */
		//acLSTM( const acLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//acLSTM& operator=( const acLSTM& src) { Timelayer::operator=(src); return *this; }
