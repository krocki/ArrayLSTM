 ...
 *
 * This is the base Array-LSTM layer
 * Describe in the report in Section 3.2
 *
 */
...
template <typename T>
class cLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		cLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::cout << "clstm levels :" << _L << std::endl;
			/*init*/
			matrix_init ( p ( W ) );
			matrix_init ( p ( U ) );
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			sync_stream ( 2 );
			//fused
			cu_elementwise_clstm_forward (	& ( s ( t, g ).cu_data[0] ),
											& ( s ( t, g2 ).cu_data[0] ),
											& ( p ( b ).cu_data[0] ),
											& ( s ( t, h ).cu_data[0] ),
											& ( s ( t, c ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_clstm_backward (	& ( g ( t, g ).cu_data[0] ),
											& ( g ( t, y ).cu_data[0] ),
											& ( s ( t, c ).cu_data[0] ),
											& ( s ( t, ct ).cu_data[0] ),
											& ( g ( t, c ).cu_data[0] ),
		...
		/* optional */
		/* copy constr */
		//cLSTM( const cLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//cLSTM& operator=( const cLSTM& src) { Timelayer::operator=(src); return *this; }
