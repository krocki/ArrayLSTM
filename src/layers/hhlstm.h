...
*
*   Implementation of hhLSTM layer
*
*/
...
template <typename T>
class hhLSTM : public Timelayer<T> {
...
public:
	...
	/* main constructor */
	hhLSTM ( size_t _M, size_t _N, size_t _B,
...

	{
		...
		std::make_tuple ( "g", _B, 4 * _N * 1 ),
		std::make_tuple ( "g2", _B, 4 * _N * 1 )

	}, {

		/* define params */
		std::make_tuple ( "W", _M, 4 * _N * 1 ),
		std::make_tuple ( "U", _N, 4 * _N * 1 ),
		std::make_tuple ( "b", 1, 4 * _N * 1 ),

		std::make_tuple ( "B_ones", _B, 1 )

	} ) {

		std::cout << "hhlstm levels :" << _L << std::endl;
		...
		p ( b ).block_forall ( 2 * N * 1, N * 1, [ = ] () { return 1; } );
...
#define c_gates 3 * N * 1 * s(t, g).rows()
#define i_gates 0 * N * 1 * s(t, g).rows()
#define f_gates 2 * N * 1 * s(t, g).rows()
#define o_gates 1 * N * 1 * s(t, g).rows()
	...
	virtual void forward ( bool apply_dropout, size_t t = 1 ) {
		...
		sync_stream ( 2 );
		//fused
		cu_elementwise_hhlstm_forward (	& ( s ( t, g ).cu_data[0] ),
										& ( s ( t, g2 ).cu_data[0] ),
										& ( p ( b ).cu_data[0] ),
										& ( s ( t, h ).cu_data[0] ),
										& ( s ( t, c ).cu_data[0] ),
	...
	virtual void backward ( bool apply_dropout, size_t t ) {
		...
		cu_elementwise_hhlstm_backward (	& ( g ( t, g ).cu_data[0] ),
											& ( g ( t, y ).cu_data[0] ),
											& ( s ( t, c ).cu_data[0] ),
											& ( g ( t, c ).cu_data[0] ),
											& ( s ( t, g ).cu_data[0] ),
	...
	/* optional */
	/* copy constr */
	//hhLSTM( const hhLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }

	/* assignent operator */
	//hhLSTM& operator=( const hhLSTM& src) { Timelayer::operator=(src); return *this; }
